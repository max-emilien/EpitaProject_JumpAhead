...
namespace UnityStandardAssets.Characters.ThirdPerson
{
	...
	public class ThirdPersonCharacter : MonoBehaviour
	{
		...
		private float time_start_dash;
		private float time_start_rechargement_dash;
		...
		void Update()
		{
			if (Input.anyKey) {
				...
				if ((possibilite_dash)&&(Input.GetKey(KeyCode.A))&&(!in_dash)&&(!rechargement_dash)) // dash
				    {
					if (m_IsGrounded)
					{
						m_Rigidbody.transform.position = new Vector3(m_Rigidbody.transform.position.x, m_Rigidbody.transform.position.y +1, m_Rigidbody.transform.position.z);
			...
			if (in_dash) //boucle pendant le dash
			{
				in_dash = time_start_dash>=Time.time-0.5;
				if (!in_dash)
				{
					rechargement_dash = true;
					time_start_rechargement_dash = Time.time;
				}
			}
